...
ExpandedElementsStore.prototype = {
	...
	register: function () {
		this._dispatcher.register(ActionTypes.EXPAND_ELEMENT,
			this.onExpand, this);
		this._dispatcher.register(ActionTypes.COLLAPSE_ELEMENT,
	...
	onExpand: function (action) {
		this.expandedElements = this.expandedElements.add(
			this._identifier(action.element)
		);
		this.update(action.element);
	},

	onCollapse: function (action) {
		this.expandedElements = this.expandedElements.remove(
			this._identifier(action.element)
		);
		this.update(action.element);
	},
