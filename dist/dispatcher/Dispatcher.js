...
Dispatcher.prototype.unregister = function (actionId, callback) {
	assert(this._callbackExists(actionId, callback), "Dispatcher.unregister(...): This callback was never registered with id `" + actionId + "`");
	this._callbacks[actionId].splice(F.findIndex(F.propEq("callback", callback), this._callbacks[actionId]), 1);
};
...
Dispatcher.prototype.waitFor = function (stores) {
	...
	for (var i = 0; i < stores.length; i++) {
		var index = F.findIndex(F.propEq("store", stores[i]), this._callbacks[this._currentActionId()]);
		...
		if (this._isPending[index]) {
			assert(this._isHandled[index], "Dispatcher.waitFor(...): Circular dependency detected while " + "waiting within action `" + this._callbacks[this._currentActionId()] + "`.");
			continue;
		}
...
Dispatcher.prototype.dispatch = function (payload) {
	assert(!this._isDispatching, "Dispatch.dispatch(...): Cannot dispatch in the middle of a dispatch.");
	assert(payload && payload.type, "Dispatch.dispatch(...): Cannot dispatch payload without a type.");
...
Dispatcher.prototype._currentActionId = function () {
	assert(this._isDispatching && this._pendingPayload, "Dispatch._dispatchingActionId(...): " + "Can only have a dispatched action during dispatch.");
...
Dispatcher.prototype._invokeCallback = function (index) {
	assert(this._isDispatching, "Dispatch._dispatchingActionId(...): Can invoke callback during dispatch");
	assert(this._callbacks[this._currentActionId()][index], "Dispatch._dispatchingActionId(...): Cannot invoke `" + this._currentActionId() + "` callback with index " + index);

	this._isPending[index] = true;
	var callbackObj = this._callbacks[this._currentActionId()][index];
	callbackObj.callback.call(callbackObj.store, this._pendingPayload);
...
Dispatcher.prototype._callbackExists = function (actionId, callback) {
	return this._callbacks[actionId] && this._callbacks[actionId].some(F.propEq("callback", callback));
};
