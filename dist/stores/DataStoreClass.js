...
DataStore.prototype = {
  ...
  */
	...
	/* private */
	_register: function () {
		this._dispatcher.register(ActionTypes.SET_DATA, this._onFormLoaded, this);
		this._dispatcher.register(ActionTypes.DELETE_ELEMENT, this._onDeleteElement, this);
	...
	_onDragEnd: function (action) {
		...
		if (action.target && !action.source) {
			...
			this._data = this._data.withMutations(function (form) {
				...
				return form.setIn(sourceKeyPath, action.source.after).setIn(targetKeyPath, action.target.after);
			});
		}
		this._emitData();
	},
